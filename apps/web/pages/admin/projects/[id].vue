...
const { get, update, remove } = useAdminProjects();
...
const form = reactive({
  status: project.value.status as ContentStatus,
  featured: project.value.featured,
  stack: [...project.value.stack],
  links: { repo: project.value.links.repo ?? "", demo: project.value.links.demo ?? "" },
  coverImage: project.value.coverImage ?? "",
  content: {
    pt: { ...project.value.content.pt },
    en: { ...project.value.content.en },
  },
});
...
async function onSubmit() {
  ...
  try {
    ...
    toast.add({ title: "Projeto salvo!", color: "success" });
  } catch {
